<?php
	...
	$query_validate=mysqli_query($con,"select client_id from purchases_order where client_id='".$id."'");
	$count=mysqli_num_rows($query_validate);
		if ($count==0)
		{
			if($delete=mysqli_query($con, "DELETE FROM clients WHERE id='$id'") and $delete2=mysqli_query($con,"DELETE FROM contacts_clients WHERE client_id='$id' ")){
				$aviso="Bien hecho!";
		...
		{
			$aviso="¡Aviso!";
			$msj="Error al eliminar los datos. El cliente se encuentra vinculado al modulo de compras";
			$classM="alert alert-danger";
			$times="&times;";
		}
...
if($action == 'ajax'){
	...
	include 'pagination.php'; //Incluye el archivo de paguinación
	//Validación de paguinación
				...
				<div class="box-body">
					...
					<table class="table table-condensed table-hover">
						...
						while($row = mysqli_fetch_array($query)){	
							...
							$fecha=$d."-".$m."-".$Y;
							$sql_contacto=mysqli_query($con,"select first_name, last_name, phone, email from contacts_clients where client_id='$id'");
						...
						<tr>
							...
							<td>
								...
								<br><i class=''></i> <a href="http://<?php echo $replace_http;?>" target="_blank"> <?php echo $website;?></a>
								<?php } ?>
							</td>
							<td>
								<?php
							...
							<td>
								<i class=''></i> <?php echo $contact;?><br>
								<?php if (!empty($phone)){?>
								<i class=''></i> <?php echo $phone;?>
								<?php }?>
								<?php if (!empty($email)){?>
								<br><i class=''></i> <a href="mailto:<?php echo $email;?>"><?php echo $email;?></a>
								<?php }?>
							</td>
