...
$stock_able=mysqli_query($con, "SELECT stock FROM products WHERE product_id='$id'");
$rw_stock=mysqli_fetch_array($stock_able);
$stock=$rw_stock["stock"];
/*echo(5);
echo($stock);
echo($qty);*/
if($stock >= $qty){
	...
	$cantidad=$stock-$qty;
	$insert_decrement=mysqli_query($con, "UPDATE products SET stock='$cantidad' WHERE product_id='$id'");
	echo mysqli_error($con);
}else{
/* 	?>
	<div class="alert alert-danger" role="alert">
			<button type="button" class="close" data-dismiss="alert">&times;</button>
			<strong>No hay suficiente stock</strong>
	</div>
	<?php */
	?>
	<script>
...
{
$id=intval($_GET['id']);
$add_stock=mysqli_query($con, "SELECT qty FROM purchase_order_product WHERE id='$id'");
$rw_qty=mysqli_fetch_array($add_stock);
$qty_add=$rw_qty["qty"];
$product_able=mysqli_query($con, "SELECT product_id FROM purchase_order_product WHERE id='$id'");
$rw_product_able=mysqli_fetch_array($product_able);
$product=$rw_product_able["product_id"];
$id_product=mysqli_query($con, "SELECT product_id FROM products WHERE product_id='$product'");
$rw_id=mysqli_fetch_array($id_product);
$product_id=$rw_id["product_id"];
$stock_add=mysqli_query($con, "SELECT stock FROM products WHERE product_id='$product_id'");
$rw_stock_add=mysqli_fetch_array($stock_add);
$stock_insert=$rw_stock_add["stock"];
$cantidad_insert=$stock_insert+$qty_add;
$insert_increment=mysqli_query($con, "UPDATE products SET stock='$cantidad_insert' WHERE product_id='$product_id'");
	...
	$qty_rec=$row['qty_rec'];
		...
		<tr>
			...
			<td class='text-center col-md-1'>
				<input type="number" style="text-align:center" value="<?php echo $qty;?>" class="form-control form-rounded input-sm" onblur="actualizar_item(this.value,1,'<?php echo $id;?>')" <?php echo $disabled;?>>
			</td>
			<td><?php echo $product_name;?></td>
			<td><?php echo $nombre_sucursal;?></td>
			<td class="col-md-2">
				<input type="text"  value="<?php echo $status_oc;?>" class="form-control form-rounded input-sm" onblur="actualizar_item(this.value,5,'<?php echo $id;?>')" >
			</td>
			<td class="col-md-2">
				<input type="text" style="text-align:right" value="<?php echo number_format($unit_price,$precision_moneda,$sepador_decimal_moneda,$sepador_millar_moneda);?>" class="form-control form-rounded input-sm" onblur="actualizar_item(this.value,2,'<?php echo $id;?>')" <?php echo $disabled;?>>
			</td>
			...
			<?php 
				if($status!=3){
			?>
				<span class="pull-right"><a href="#" onclick="eliminar('<?php echo $id ?>')"><i class=" btn btn-danger btn-rounded glyphicon glyphicon-trash"></i></a></span>
			<?php 
				}
			?>
			</td>
...
<tr>
	<td colspan=6><span class="pull-right">IVA<?php (tax_txt);?><?php echo " $simbolo_moneda";?></span></td>
