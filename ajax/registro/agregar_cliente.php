...
if (empty($_POST['bussines_name'])){
			...
			// escaping, additionally removing everything that could be (html/javascript-) code
				...
				if (!empty($tax_number)){
					$sql_customer=mysqli_query($con,"select * from clients where tax_number='$tax_number'");
				...
				if ($count==0){
					...
					//Escribir nuevos datos en la base de datos
                    $sql = "INSERT INTO clients (created_at,name,address1,city,state,postal_code,	country_id, work_phone, website, tax_number) VALUES('$created_at','$name','$address1','$city','$state','$postal_code', '$country_id','$work_phone','$website','$tax_number');";
                    $query = mysqli_query($con,$sql);

                    // 	Si se ha añadido con éxito
                    if ($query) {
                        $messages[] = "El cliente ha sido creado con éxito.";
						$last=mysqli_query($con,"select LAST_INSERT_ID(id) as last from clients order by id desc limit 0,1 ");
						$rw=mysqli_fetch_array($last);
						$client_id=$rw['last'];
						$query2=mysqli_query($con,"insert into contacts_clients (client_id, first_name, last_name, 	email, phone) values ('$client_id','$first_name','$last_name','$email', '$phone')");
                    } else {
