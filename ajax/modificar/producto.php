...
if (version_compare(PHP_VERSION, '5.3.7', '<')) {
    ...
    require_once("../../libraries/password_compatibility_library.php");
		...
		if (empty($_POST['product_id'])){
		...
		} else if (empty($_POST['production_price'])){
			$errors[] = "Precio de compra está vacío";
		} else if (empty($_POST['stock'])){
			$errors[] = "El stock está vacío";
		}else if (empty($_POST['selling_price'])){
			$errors[] = "Precio de venta está vacío";
		} else if(preg_match($pattern, $_POST['production_price']) == '0'){
			$errors[] = "Precio de producción tiene un formato inválido. Asegurate que sea un número con 2 decimales";
			...
			// escaping, additionally removing everything that could be (html/javascript-) code
				...
				$production_price= mysqli_real_escape_string($con,(strip_tags($_POST["production_price"],ENT_QUOTES)));
				...
				// update data
					...
					note='".$note."',status='".$status."', manufacturer_id='".$manufacturer_id."', category_id='".$category_id."', production_price='".$production_price."', stock='".$stock."', selling_price='".$selling_price."', profit='".$profit."' WHERE product_id='$product_id' ";
