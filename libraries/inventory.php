<?php
	...
	function update_production_price($product_id,$production_price){
		global $con;//Variable de conexion
		$update=mysqli_query($con,"UPDATE products SET production_price='".$production_price."' WHERE product_id='".$product_id."'");
	}
	...
	function update_selling_price($product_id,$production_price){
		global $con;//Variable de conexion
		$sql=mysqli_query($con,"SELECT profit from products where product_id='$product_id'");
		$rw=mysqli_fetch_array($sql);
		$utilidad=intval($rw['profit']);

		$utilidad=($production_price * $utilidad) /100;
		$precio_venta=$production_price + $utilidad;
