...
<page backtop="20mm" backbottom="15mm" backleft="15mm" backright="15mm" style="font-size: 13px; font-family: helvetica" backimg="">
	<?php 
	$title_report='Reporte de Órdenes de compras';
	include('page_header_footer.php');
	?>

	
	<div style='border-bottom: 3px solid #2ecc71;padding-bottom:10px; padding-top:50px;'>
		Usuario:  
		<?php
    ...
    <table class="table-bordered" style="width:100%;" cellspacing=0>
        <tr>
			<th class='top bottom'  style="width: 12%;text-align:center">Número Orden</th>
            <th class='top bottom'  style="width: 20%;">Cliente</th>
			<th class='top bottom'  style="width: 20%;">Dirección</th>
			...
			<th class='top bottom'  style="width: 12%;text-align:right">IVA<?php (tax_txt);?></th>
		...
		while($row=mysqli_fetch_array($query)){
			...
			$sql_client=mysqli_query($con,"select * from clients where id='".$client_id."'");
			$rw_client=mysqli_fetch_array($sql_client);
			$client_name=$rw_client['name'];
			$client_address=$rw_client['address1'];
			...
			?>
				<tr>
					...
					<td class='bottom' style="width: 20%;"><?php echo $client_name;?></td>
					<td class='bottom' style="width: 20%;"><?php echo $client_address;?></td>
		...
		?>
		<tr>	<td style='text-align:right;border-top:3px solid #2ecc71;padding:4px;padding-top:4px;font-size:14px'></td>
